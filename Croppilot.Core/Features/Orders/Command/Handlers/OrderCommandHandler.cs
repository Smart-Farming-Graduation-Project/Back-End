...
{
    public async Task<Response<string>> Handle(CreateOrderCommand command, CancellationToken cancellationToken)
    {
        var totalAmount = 0m;
        ...
        return result == OperationResult.Success
            ? Created<string>("Order created successfully")
            : BadRequest<string>("Order creation failed");
    }
