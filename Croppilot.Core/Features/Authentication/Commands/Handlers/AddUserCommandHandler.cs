...
{
    internal class AddUserCommandHandler(IAuthenticationService service)
        : ResponseHandler, IRequestHandler<AddUserCommand, Response<string>>
    {
        public async Task<Response<string>> Handle(AddUserCommand request, CancellationToken cancellationToken)
        {
            if (await service.GetUserByEmail(request.Email) is not null)
                return BadRequest<string>("Email must be unique");
            if (await service.GetUserByUserName(request.UserName) is not null)
